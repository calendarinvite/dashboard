...
def get_organizer(request):
    ...
    If base64, request body is decoded. Request body is loaded as
    dictionary and the 'organizer' value is returned as str.
    """
    if request.get("isBase64Encoded", False):
        request["body"] = b64decode(request["body"]).decode("utf-8-sig")

    return json.loads(request["body"])["organizer"]
